...
(function lnReportPeriodModule() {
  ...
  // Parse a period string like "يناير 2025 | January 2025" -> { m, y }.
  function parsePeriod(s) {
    ...
    for (var i = 0; i < AR_MONTHS.length; i++) {
      if (ar.indexOf(AR_MONTHS[i]) >= 0) { mi = i; break; }
    }
    if (mi < 0) {
      for (var j = 0; j < EN_MONTHS.length; j++) {
        if (ar.toLowerCase().indexOf(EN_MONTHS[j].toLowerCase()) >= 0) { mi = j; break; }
      }
    }
    if (mi < 0) return null;
    return { m: mi, y: y };
  ...
  // All months that actually have data: read from the report's own period
  // selects (#month1/#month2 options = recorded periods), the period chips,
  // the periods blob, and any month names rendered in the report.
  function getRecordedMonths() {
    var map = {};
    function add(p) { if (p) map[p.y + "-" + p.m] = p; }
    ...
    texts.forEach(function (txt) {
      var T = toEn(txt);
      for (var i = 0; i < AR_MONTHS.length; i++) {
        var re = new RegExp(escapeRe(AR_MONTHS[i]) + "\\s*((?:19|20)\\d{2})", "g");
        var mm;
        while ((mm = re.exec(T)) !== null) add({ m: i, y: parseInt(mm[1], 10) });
      }
    });

    var out = Object.keys(map).map(function (k) { return map[k]; });
    out.sort(function (a, b) { return b.y - a.y || b.m - a.m; });
  ...
  function recordedYears(months) {
    var years = {};
    (months || []).forEach(function (mo) { years[mo.y] = true; });
  ...
  // Build a bilingual label for a chosen period pick.
  function buildLabel(level, pick) {
    if (!pick) return null;
    if (level === "month") {
      if (pick.m == null || pick.y == null) return null;
      return { ar: AR_MONTHS[pick.m] + " " + pick.y, en: EN_MONTHS[pick.m] + " " + pick.y };
    }
    if (level === "quarter") {
      if (pick.q == null || pick.y == null) return null;
      return { ar: AR_QUARTERS[pick.q] + " " + pick.y, en: "Q" + (pick.q + 1) + " " + pick.y };
    }
    if (pick.y == null) return null;
    return { ar: String(pick.y), en: String(pick.y) };
  }

  // Split a select value "AR | EN" into its parts.
  function valParts(v) {
    if (!v) return null;
    v = String(v).trim();
    if (!v) return null;
    var parts = v.split("|");
    return { full: v, ar: (parts[0] || "").trim(), en: (parts[1] || "").trim() };
  }

  // Build [baseline, replacement] string pairs from the report's canonical
  // current/previous period strings (#month1 / #month2).
  function buildPairs(sel) {
    ...
    function push(base, lab) {
      if (!base || !lab) return;
      if (base.full && lab.ar && lab.en) pairs.push([base.full, lab.ar + " | " + lab.en]);
      if (base.ar) pairs.push([base.ar, lab.ar]);
      if (base.en) pairs.push([base.en, lab.en]);
    }
    push(b1, lc);
    push(b2, lp);
    pairs = pairs.filter(function (p) { return p[0] && p[1] && p[0] !== p[1]; });
  ...
  // Replace baseline period names inside a text node, wrapping each
  // replacement in a marker span so it can be re-applied or cleared cleanly.
  function processTextNode(node, pairs) {
    ...
    while (i < s.length) {
      ...
      for (var k = 0; k < pairs.length; k++) {
        ...
        if (idx >= 0 && (best < 0 || idx < best)) { best = idx; bp = pairs[k]; }
      }
      if (best < 0) { frag.appendChild(document.createTextNode(s.slice(i))); break; }
  ...
  function relabel(sel) {
    ...
    while ((n = walker.nextNode()) !== null) {
      ...
      var has = false;
      for (var k = 0; k < pairs.length; k++) {
        if (n.nodeValue.indexOf(pairs[k][0]) >= 0) { has = true; break; }
      }
      if (has) nodes.push(n);
    }
    ...
    if (host) {
      ...
      for (var i = 0; i < spans.length; i++) {
        ...
        sp.parentNode.replaceChild(
          document.createTextNode(sp.getAttribute("data-ln-orig") || sp.textContent),
          sp
        );
      }
    }
  ...
  function openModal() {
    ...
    function ensureDefaults() {
      if (draft.level === "month") {
        ...
        if (!draft.current || draft.current.q == null) draft.current = { q: cq, y: years[0] };
        if (!draft.comparison || draft.comparison.q == null) draft.comparison = { q: (cq + 3) % 4, y: years[0] };
      } else {
        if (!draft.current || draft.current.y == null || draft.current.m != null || draft.current.q != null) {
          draft.current = { y: years[0] };
        }
        if (!draft.comparison || draft.comparison.y == null || draft.comparison.m != null || draft.comparison.q != null) {
    ...
    function pickerHtml(level, target) {
      if (level === "month") {
        return '<div class="ln-period-grid ln-period-mgrid" data-target="' + target + '" data-kind="month">' +
          months.map(function (mo) {
            return '<div class="ln-period-opt" data-m="' + mo.m + '" data-y="' + mo.y + '">' + AR_MONTHS[mo.m] + ' ' + mo.y + '</div>';
          }).join("") +
          '</div>';
      }
      if (level === "quarter") {
        var qs = "";
        for (var q = 0; q < 4; q++) qs += '<div class="ln-period-opt" data-q="' + q + '">' + AR_QUARTERS[q] + '</div>';
        var ys = "";
        for (var i = 0; i < years.length; i++) ys += '<div class="ln-period-opt" data-y="' + years[i] + '">' + years[i] + '</div>';
        return '<div class="ln-period-grid" data-target="' + target + '" data-kind="q">' + qs + '</div>' +
          '<p class="ln-period-label">السنة</p>' +
          '<div class="ln-period-grid" data-target="' + target + '" data-kind="qy">' + ys + '</div>';
      }
      var yy = "";
      for (var j = 0; j < years.length; j++) yy += '<div class="ln-period-opt" data-y="' + years[j] + '">' + years[j] + '</div>';
      return '<div class="ln-period-grid" data-target="' + target + '" data-kind="y">' + yy + '</div>';
    }

    function subLabel(target) {
      if (draft.level === "month") return target === "current" ? "الشهر الحالي" : "شهر المقارنة";
      if (draft.level === "quarter") return target === "current" ? "الربع الحالي" : "ربع المقارنة";
      return target === "current" ? "السنة الحالية" : "سنة المقارنة";
    }
    ...
    function wirePickers(box) {
      ...
      Array.prototype.forEach.call(opts, function (o) {
        ...
        var active = false;
        if (kind === "month") active = (parseInt(o.getAttribute("data-m"), 10) === cur.m && parseInt(o.getAttribute("data-y"), 10) === cur.y);
        else if (kind === "q") active = (parseInt(o.getAttribute("data-q"), 10) === cur.q);
        else if (kind === "qy") active = (parseInt(o.getAttribute("data-y"), 10) === cur.y);
        else if (kind === "y") active = (parseInt(o.getAttribute("data-y"), 10) === cur.y);
        if (active) o.classList.add("active");
        o.addEventListener("click", function () {
          var t = draft[target] || {};
          if (kind === "month") {
            t = { m: parseInt(o.getAttribute("data-m"), 10), y: parseInt(o.getAttribute("data-y"), 10) };
          } else if (kind === "q") {
    ...
    function renderDetail() {
      ...
      box.innerHTML =
        '<div class="ln-period-sub">🟢 ' + subLabel("current") + '</div>' +
        pickerHtml(draft.level, "current") +
        '<div class="ln-period-div"></div>' +
        '<div class="ln-period-sub cmp">🔵 ' + subLabel("comparison") + '</div>' +
